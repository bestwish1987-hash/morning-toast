...
每首都會有：主旋律、和弦、貝斯、鼓（musicbox 沒有鼓）、鋪底（只有 pop、waltz 預設開，頁面上可以開關）。

頁面的混音台裡每一軌可以：
- 換樂器：和弦（鋼琴、電鋼琴、吉他、八音盒、合成器、弦樂）、貝斯（貝斯、合成器、八音盒、鋼琴）、鋪底（合成鋪底、弦樂、不要）
- 調音量、左右定位、音色（暗↔亮）、殘響量、靜音
- 「加一軌」最多 4 軌：跟著主旋律（可高低八度，例如弦樂低八度疊旋律）或第二層和弦（琶音、快琶音、每拍齊奏、反拍、刷弦、長音）
- 母帶預設：原味、溫暖、明亮、Lo-fi、廣播、大場地；最後都經過壓縮和限幅，不會爆音

建置時 `--chords` `--bass` `--pad` 可以先設好伴奏樂器；混音台的設定存在使用者的瀏覽器裡。
MIDI 匯出時各是一軌（含加軌），音量和左右會寫成 CC7／CC10，鼓在第 10 通道；分軌匯出是每軌一個 WAV 包成 zip。
